...
% pause(0.1); object = f_entropyExtract(obj, frame);
...
function SIG = entropySig(IMAGE, RADIUS)
    ...
    for i = (r+1):r:size(IMAGE, 1)-(r+1)
        for j = (r+1):r:size(IMAGE, 2)-(r+1)
            n = IMAGE(i-r:i+r, j-r:j+r);
            SIG(i-r:i+r, j-r:j+r) = entropy(n);
        end
    end
end
